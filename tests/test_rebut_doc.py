...
def test_rebut_doc_success(mock_run, temp_workdir):
    ...
    mock_run.assert_any_call(["fct_csvbib", "label.csv", "-n"], cwd=temp_workdir, check=True)
    mock_run.assert_any_call(["fct_rebut", str(temp_workdir / "label_table.bib")], cwd=output_path, check=True)
