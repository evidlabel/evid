...
@needs_typst
def test_slash_then_nbsp_line_compiles(tmp_path):
    """A no-break space after a line-leading '/' (nature.com pricing:
    '/<NBSP>30<NBSP>days') also starts a Typst term item and must be escaped."""
    cleaned = clean_text_for_typst("before\n/\u00a030\u00a0days\nafter")
    ok, err = _typst_compiles(cleaned, tmp_path)
    assert ok, f"typst compile failed: {err}"
...
@needs_typst
def test_nbsp_indented_slash_compiles(tmp_path):
    """A no-break-space-indented '/ word' line also triggers the term-list parser."""
    cleaned = clean_text_for_typst("before\n\u00a0/ word\nafter")
    ok, err = _typst_compiles(cleaned, tmp_path)
    assert ok, f"typst compile failed: {err}"
