...
from evidmgr.models import Tag, TagItem
...
class TagService:
    ...
    def create_tag(self, name: str, owner_set: str) -> Tag:
        ...
        tag = Tag(name=name, owner_set=owner_set, created=datetime.now(tz=timezone.utc))
        tags.append(tag)
        self._save_all(tags)
        return tag
    ...
    def remove_item(self, tag_name: str, set_slug: str, doc_uuid: str) -> None:
        ...
        if tag is None:
            return
        tag.items = [i for i in tag.items if not (i.set_slug == set_slug and i.doc_uuid == doc_uuid)]
        self._save_all(tags)
