...
def assemble_subtree(
    ...
    sections: list[Section] = []
    for layer in path:
        sections.extend(_assemble_single_layer(layer, corpus_index, recipe_dir, warnings))
...
def _assemble_single_layer(
    ...
    tokens_joined = ", ".join(layer.evidence_tokens) if layer.evidence_tokens else "(none)"
    anchor_ev = f"### [{layer.layer_id}] evidence: {tokens_joined} ({len(doc_texts)} docs) ###"
    ev_parts = [anchor_ev]
...
def _resolve_evidence(
    ...
    for token in tokens:
        ...
        for text in candidates:
            key = id(text) if isinstance(text, str) else hash(text)
