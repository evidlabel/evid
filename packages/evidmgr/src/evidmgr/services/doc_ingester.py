...
class DocIngester:
    ...
    def ingest(
        ...
        if doc_dir.exists():
            logger.info("Already ingested: %s in '%s' — skipping", doc_uuid, evidence_set.slug)
        ...
        auto_title, auto_authors, auto_date = extract_pdf_metadata(original_pdf, pdf_path.name)
        logger.debug("Auto-extracted: title=%r authors=%r date=%r", auto_title, auto_authors, auto_date)
        ...
        logger.info("Metadata: label=%r title=%r authors=%r", doc_label, doc_title, doc_authors)
        ...
        try:
            ...
            logger.debug("Generated label.typ (%d bytes)", typ_path.stat().st_size if typ_path.exists() else 0)
        except Exception:
    ...
    def index_existing(
        ...
        if not typ_text:
            logger.warning("No .typ text for %s; indexing with empty content", doc_dir.name)
        else:
            logger.debug("Embedding %d chars for existing doc %s", len(typ_text), doc_dir.name)

        logger.info("Indexing existing doc %s into '%s'", doc_dir.name, evidence_set.slug)
        try:
            self.vec_service.index_document(doc, typ_text, evidence_set)
        except Exception:
        ...
        logger.info("Indexed existing doc %s into set '%s'", doc_dir.name, evidence_set.slug)
        return True
