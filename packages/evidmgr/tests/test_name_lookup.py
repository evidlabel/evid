...
from evidmgr.models import Gender, NameOrigin
from evidmgr.services.name_lookup import (
    detect_gender_from_name,
    generate_all_profiles,
    generate_fake_name,
    _load_names,
    _build_standard_profiles,
    _invert_gender,
    _invert_origin,
)
...
def test_generate_fake_name_different_profiles():
    ...
    n_inv = generate_fake_name(0, "uuid-1", Gender.FEMALE, NameOrigin.ARABIC, "gender_inverted")
...
def test_generate_all_profiles():
    ...
    assert set(profiles.keys()) == {"same", "gender_inverted", "ethnicity_inverted", "both_inverted"}
