...
from evid.models import FakeProfile, Gender, NameOrigin, seeded_rng
...
def _build_standard_profiles(
    detected_gender: Gender, detected_origin: NameOrigin
) -> list[FakeProfile]:
    ...
    return [
        FakeProfile(
            ...
            if detected_gender != Gender.UNKNOWN
            else Gender.NEUTRAL,
            target_origin=detected_origin
            if detected_origin != NameOrigin.UNKNOWN
            else NameOrigin.DANISH,
        ),
        FakeProfile(
            key="gender_inverted",
...
def detect_gender_from_name(name: str) -> Gender:
    ...
    try:
        ...
        first_name = name.split(maxsplit=1)[0] if name.strip() else name
...
def generate_all_profiles(
    ...
    return {
        p.key: generate_fake_name(
            entity_idx, doc_uuid, p.target_gender, p.target_origin, p.key
        )
        for p in profiles
    }
