...
if TYPE_CHECKING:
    from evid.models import AnonMode, AnonYaml, EvidenceSet
...
class AnonService:
    ...
    def run_extract(
        ...
        iso = datetime.now(tz=UTC).strftime("%Y%m%dT%H%M%S")
    ...
    def list_yamls(self, evidence_set: EvidenceSet) -> list[AnonYaml]:
        ...
        from evid.models import AnonYaml
        ...
        for yml_file in sorted(anon_dir.glob("*_entities.yml"), reverse=True):
            try:
                ...
                result.append(
                    AnonYaml(
                        path=yml_file,
                        generated=generated or datetime.now(tz=UTC),
    ...
    def pseudonymize(
        ...
        for entity in yaml_obj.entities:
            ...
            if isinstance(entity, dict):
                replacement = (
                    entity.get(field) or entity.get("placeholder") or ""
                ).strip()
                original = entity.get("original", "")
                variants = entity.get("variants", [])
            else:
                replacement = (
                    getattr(entity, field, None) or entity.placeholder or ""
                ).strip()
