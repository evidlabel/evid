"""ImportService — import an existing evid directory into a set.

Current layout:
    {data_dir}/sets/{slug}/docs/{uuid}/info.yml

Pre-sets (legacy) layout:
    {db_dir}/{dataset}/{uuid}/info.yml
"""
...
def _uuid_dirs_in(folder: Path) -> list[Path]:
    if not folder.is_dir():
        return []
    return [d for d in sorted(folder.iterdir()) if _is_uuid_dir(d)]


def _scan_sets_root(root: Path) -> dict[str, list[Path]]:
    ...
    for set_dir in sorted(root.iterdir()):
        if not set_dir.is_dir():
            continue
        uuid_dirs = _uuid_dirs_in(set_dir / "docs")
        if uuid_dirs:
            datasets[set_dir.name] = uuid_dirs
    return datasets


def _scan_evid_dir(evid_dir: Path) -> dict[str, list[Path]]:
    """Return {dataset_name: [uuid_dir, …]}.

    Tries current layout first:
        {data_dir}/sets/{slug}/docs/{uuid}/info.yml
        {sets_dir}/{slug}/docs/{uuid}/info.yml
    then the pre-sets layout:
        {db_dir}/{dataset}/{uuid}/info.yml
    """
    found = _scan_sets_root(evid_dir / "sets")
    if found:
        return found
    found = _scan_sets_root(evid_dir)
    if found:
        return found
    datasets: dict[str, list[Path]] = {}
    for dataset_dir in sorted(evid_dir.iterdir()):
        if not dataset_dir.is_dir():
            continue
        uuid_dirs = _uuid_dirs_in(dataset_dir)
        if uuid_dirs:
...
def import_evid_dir_single(
    ...
    UUID dirs may be direct children, or under ``docs/`` (current set layout).
    """
    if progress is None:
        progress = lambda done, total, msg: logger.info("[%d/%d] %s", done, total, msg)

    uuid_dirs = _uuid_dirs_in(evid_dir)
    if not uuid_dirs:
        uuid_dirs = _uuid_dirs_in(evid_dir / "docs")
    if not uuid_dirs:
