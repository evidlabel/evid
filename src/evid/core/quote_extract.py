...
class QuotesFile(BaseModel):
    ...
    ``job`` and ``model`` are optional stamps of intent. A recorded pass file
    is a valid input: extra ledger fields (id, timestamp, results, …) are
    ignored so ``--from machine/<pass>.json`` replays the candidates.
    """
