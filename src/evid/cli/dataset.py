...
def track_dataset(directory: Path, dataset: str = None) -> None:
    ...
    try:
        ...
        # Track sources of truth (set.yml, info.yml, label.typ, *_meta.yml and
        # the source PDFs); ignore everything evid regenerates. The regenerated
        # files (vector index, typst-query bib/json, extracted text, compile
        # byproducts) only add churn and bloat — and hubs are append-only, so a
        # blob committed once can never be removed. Rebuild them per-machine
        # with `evid set reindex` / `evid set gather`.
        gitignore_content = """# evid set: track sources, ignore anything evid regenerates.
# Vector index — rebuilt by `evid set reindex`.
vecdb/
# Scratch reasoning.
reasoning/
...
"""
        ...
        repo.git.add(A=True)
        repo.index.commit("Initial commit: track dataset contents")
