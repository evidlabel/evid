...
app = cli(
    ...
    options=[
        option(
            flags=["-d", "--db"],
            arg_type=str,
            # Display default only — runtime --db is parsed above and falls
            # back to EvidConfig.load().data_dir (which still honours a
            # leftover ~/.local/share/evidmgr).
            default="~/.local/share/evid",
    ...
    command(
        ...
        options=[
            ...
            option(
                flags=["--model"],
                arg_type=str,
                help="LLM that ran this pass (or EVID_QUOTE_MODEL)",
            ),
            option(
                flags=["--job"],
                arg_type=str,
                help="Natural-language description of this pass",
            ),
        ],
    )
)
...
doc_group.commands.append(
    command(
        name="passes",
        help="List machine-quote passes for a document (job ledger, no quote text)",
        callback=passes_callback,
        options=[
            _DATASET_OPTION,
            option(flags=["-u", "--uuid"], arg_type=str, help="Document UUID"),
            _FORMAT_OPTION,
