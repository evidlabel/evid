...
class ConfigModel(BaseModel):
    """Schema for ``~/.evidrc`` (``evid config show`` / ``update``).

    Only ``editor`` is consumed at runtime. ``default_dir``, ``directory``,
    and ``latex`` are vestigial and kept so existing files still parse.
    """
