...
class SearchTab(QWidget):
    ...
    def _run_meta_search(self) -> None:
        ...
        if self._search_busy:
            ...
            return
        from evid.gui.workers import MetaSearchWorker, track_worker
    ...
    def _run_text_search(self) -> None:
        ...
        from evid.gui.workers import FullTextSearchWorker, track_worker
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        QToolTip.hideText()
        # customContextMenuRequested pos is viewport coordinates.
        ...
        if single:
            ...
            act_copy_uuid = None
            act_copy_uuids = menu.addAction(f"Copy UUIDs ({len(uuids)})")
            menu.addSeparator()
        ...
        if action is act_label:
            ...
            QApplication.clipboard().setText(",".join(uuids))
            self.window().statusBar().showMessage(
                f"Copied {len(uuids)} UUIDs to clipboard", 3000
            )
