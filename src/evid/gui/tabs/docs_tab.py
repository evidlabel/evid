...
class DocsTab(QWidget):
    def __init__(
        ...
        self._label_keys_list.setSelectionMode(
            QListWidget.SelectionMode.ExtendedSelection
        )
        self._label_keys_list.currentItemChanged.connect(self._on_label_key_selected)
        self._label_keys_list.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self._label_keys_list.customContextMenuRequested.connect(
            self._on_label_keys_context_menu
        )
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        QToolTip.hideText()
        # customContextMenuRequested pos is viewport coordinates (event is
        # forwarded from the viewport). Remapping as widget coords subtracts
        # the header height and selects a row above the cursor.
        ...
        if action is act_label:
            ...
            from evid.core.prompt import quotes_yaml

            md = quotes_yaml([d.path for d in docs])
            if md:
    ...
    def _on_selection_changed(self) -> None:
        ...
        try:
            model = InfoModel.model_validate(
                {**info, "uuid": info.get("uuid") or doc.uuid}
            )
        except Exception:
            logger.exception("Failed to parse info.yml for %s", doc.uuid)
            model = InfoModel(uuid=doc.uuid)
        ...
        self._detail_title.setText(model.title or "")
        self._detail_authors.setText(model.authors or "")
        self._detail_dates.setText(model.dates or "")
        self._detail_tags.setText(model.tags or "")
        self._detail_label.setText(model.label or "")
        self._detail_url.setText(model.url or "")
        self._detail_notes.setPlainText(meta.get("notes", ""))
        carried = set(parse_tags_field(model.tags))
        self._pill_pool.set_carried_tags(carried)
    ...
    def _on_add_from_url(self) -> None:
        ...
        from evid.gui.workers import UrlFetchWorker, track_worker
        ...
        track_worker(self._workers, worker, worker.ready, worker.error)
        worker.start()
    ...
    def _load_label_keys(self, doc: Document) -> None:
        ...
        self._label_key_text.clear()
        try:
            pairs = label_entries(doc.path)
        except Exception:
            logger.exception("Could not read label.json for %s", doc.uuid)
            return
    ...
    def _on_label_keys_context_menu(self, pos) -> None:
        items = self._label_keys_list.selectedItems()
        if not items:
            item = self._label_keys_list.itemAt(pos)
            if item is None:
                return
            items = [item]
        menu = QMenu(self)
        label_txt = "Copy label" if len(items) == 1 else f"Copy {len(items)} labels"
        act_copy = menu.addAction(label_txt)
        action = menu.exec(self._label_keys_list.viewport().mapToGlobal(pos))
